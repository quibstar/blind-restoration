defmodule BlindShopWeb.UserAuthTest do
  use BlindShopWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias BlindShop.Accounts
  alias BlindShop.Accounts.Scope
  alias BlindShopWeb.UserAuth

  import BlindShop.AccountsFixtures
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        BlindShopWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
