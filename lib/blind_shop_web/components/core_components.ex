defmodule BlindShopWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: BlindShopWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(BlindShopWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(BlindShopWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  def dropdown(assigns) do
    ~H"""
    <div class="relative">
      <div phx-click={show("##{@id}")}>
        {render_slot(@trigger)}
      </div>
      <div
        id={@id}
        class="absolute right-0 top-full mt-2 w-auto py-1 bg-base-100 rounded-md shadow-lg ring-opacity-5 z-50 hidden"
